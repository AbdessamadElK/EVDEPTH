...
class UnetAdaptiveBins(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        unet_out = self.decoder(encoder_out, **kwargs)
    ...
    def build(cls, n_bins, **kwargs):
        ...
        # Change first convolution to take 15 channels as input instead of 3
        old_conv = basemodel.conv_stem
        basemodel.conv_stem = Conv2dSame(
            in_channels=15,
        ...
        nn.init.kaiming_normal_(basemodel.conv_stem.weight, mode='fan_out', nonlinearity='relu')

        if basemodel.conv_stem.bias is not None:
            nn.init.zeros_(basemodel.conv_stem.bias)
